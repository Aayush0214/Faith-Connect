...
class WorshiperChatPage extends StatefulWidget {
  final ChatEntity chat;
  const WorshiperChatPage({super.key, required this.chat});

  @override
  State<WorshiperChatPage> createState() => _WorshiperChatPageState();
}

class _WorshiperChatPageState extends State<WorshiperChatPage> {
  ...
  void initState() {
    ...
    _chatBloc.add(WatchMessagesEvent(conversationId: widget.chat.id));
    _chatBloc.add(MarkMessagesAsReadEvent(conversationId: widget.chat.id));
  }
      ...
      body: BlocListener<ChatBloc, ChatState>(
        listener: (context, state) {
          if (state is MessagesLoaded) {
            ...
            if (hasUnread) {
              context.read<ChatBloc>().add(
                  MarkMessagesAsReadEvent(conversationId: widget.chat.id));
            }
          }
        },
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 8.w),
          GestureDetector(
            onTap: () => _handleSendPressed(_controller.text),
            child: Container(
